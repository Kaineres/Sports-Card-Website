---
type: index
updated: 2026-05-19
---

# Cards wiki — index

Catalog of pages in the sports cards sub-wiki. Updated on every ingest.

## Entities

- [[psa]] — third-party trading card authentication and grading service. `[Certain]` — [[psa-grading-service]], [[psa-grading-standards]]
- [[bgs]] — Beckett Grading Services; competing third-party grader with a four-subgrade model and Pristine label hierarchy. `[Certain]` — [[bgs-grading-scale]]
- [[sgc]] — SGC; competing third-party grader publishing a 20-step single-grade scale (no subgrades, no back-centering), 1 → 10 PRI. `[Certain]` — [[sgc-grading-scale]]

## Concepts

- [[card-grading]] — what grading is: two-step (authenticate + grade), produces an encapsulated slab carrying a numeric grade or alternative label. `[Certain]` — [[psa-grading-service]], [[psa-grading-standards]], [[bgs-grading-scale]], [[sgc-grading-scale]]
- [[grading-scales]] — the 1–10 numeric grade scale across services. PSA grades 1, 1.5, 2–10 (except 8.5) fully defined; BGS integer grades + 9.5 + Pristine Black/Gold variants fully defined; **SGC 20-step scale (1 → 10 PRI) fully defined**; CGC stubbed. `[Certain]` — [[psa-grading-standards]], [[bgs-grading-scale]], [[sgc-grading-scale]]
- [[population-report]] — grade-distribution database maintained by PSA, BGS, and CGC; tracks how many copies of a card exist at each grade level. Core rarity-assessment tool; covers distortion factors (crack-and-resubmit, cross-grading, submission rate) and value implications. `[Certain]` — [[packz-psa-population-report-guide]], [[goingtwice-graded-pop-reports]], [[sundocards-psa-bgs-pop-reports]], [[yahoo-population-count-explained]], [[psa-market-report-population-perspective]]
- [[bgs-subgrades]] — BGS's four-subgrade evaluation model (centering, corners, edges, surface) plus the Pristine 10 Black/Gold label rule. `[Certain]` — [[bgs-grading-scale]]
- [[grading-qualifiers]] — defect suffixes attached to a numeric grade (PSA MK detailed; other PSA categories stubbed; BGS/SGC qualifier systems not yet ingested — BGS appears to use subgrades in lieu of qualifiers). `[Certain — partial capture]` — [[psa-grading-standards]]
- [[no-grade-outcomes]] — N1–N9 codes and Authentic / Authentic Altered encapsulation labels (PSA only; BGS and SGC no-grade taxonomies not yet ingested). `[Certain]` — [[psa-grading-standards]]
- [[grading-service-tiers]] — how grading services structure pricing: declared value × turnaround speed (PSA only; BGS and SGC pricing not yet ingested). `[Certain]` — [[psa-grading-service]]

## Sources

- [[psa-grading-service]] — PSA's public-facing services page (ingested 2026-05-18).
- [[psa-grading-standards]] — PSA's grading standards page (ingested 2026-05-18; re-ingested 2026-05-18 with grade-by-grade clipping → 11/12 numeric grades captured, PSA 8.5 still missing; supplementary re-clips 11+12 ingested 2026-05-18 → added top-level ungradable rejection rule, no contradictions).
- [[bgs-grading-scale]] — BGS's grading scale at `beckett.com/grading` (ingested 2026-05-18; grade-by-grade clipping across 12 raw "Back Button*.md" files → integer grades 1–9 + 9.5 + Pristine 10 Black/Gold variants all captured; half-points not separately defined by BGS; pricing/tier and no-grade data not captured).
- [[sgc-grading-scale]] — SGC's grading scale at `gosgc.com/card-grading/scale` (ingested 2026-05-18; grade-by-grade clipping across 21 raw "SGC Grading Scale*.md" files → all 20 grade levels (1, 1.5, 2, 2.5, …, 9.5, 10 GM, 10 PRI) captured; pricing/tier, qualifier system, and no-grade data not captured).
- [[psa-market-report-population-perspective]] — PSA's own article by Joe Orlando on the Population Report (ingested 2026-05-19); primary-source warnings on misreading low pop counts.
- [[packz-psa-population-report-guide]] — Packz Blog guide to using the PSA Population Report for investing (ingested 2026-05-19); grading difficulty thresholds, value mechanics.
- [[goingtwice-graded-pop-reports]] — Going Twice explainer covering PSA, BGS, and CGC pop reports (ingested 2026-05-19); cross-grader grade rate comparison (Umbreon VMAX Alt Art example).
- [[sundocards-psa-bgs-pop-reports]] — Sundo Cards guide to PSA and BGS pop reports (ingested 2026-05-19); cross-grading inflation, within-grade quality variation.
- [[yahoo-population-count-explained]] — Yahoo Sports / Hockey News pop count explainer (ingested 2026-05-19); "Population Higher" field definition, PSA no-half-grades at 9+, liquidity vs. pricing-power dynamics.

## Synthesis

_(none yet)_
